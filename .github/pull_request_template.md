## Related Issue
<!-- Link the issue on Project, ex: [#42](https://github.com/POA-Alema/Project/issues/42) -->

## Checklist

- [ ] No linting errors (run `npm run lint:fix`)
- [ ] No console logs (remove all `console.log` statements)
- [ ] No commented-out code or comments (remove all commented-out code)
- [ ] No loose strings (all UI text comes from i18n constants)
- [ ] No conflicts with `develop` (merge `develop` into your branch and resolve conflicts)
- [ ] Code follows the project's style guidelines

## Screenshots (when applicable)

- Adicione aqui as telas de Figma usadas como referência, especialmente as telas de mapa e o painel de informações.
- Para US22-FE86, inclua prints dos estados de: permissão negada, dentro do limite e fora do limite.

## How to Test
<!-- Provide instructions on how to test this PR -->
<!-- Example: -->
1. Open http://localhost:3000/frontend in your browser
2. Navigate to ...
3. Click X button ...
4. Observe X if Y

## Additional Notes
<!-- Any additional information that is important to this PR -->
<!-- For example, concerns, things to be aware of when reviewing, etc. -->
